...
module.exports = class Conductor extends DefaultBot {
  constructor(app) {
    ...
    this.commands.connect = (self) => {
      self.fsm.connect();
      try {
        Object.entries(self.players).forEach(([playerKey, player]) => {
    ...
    this.commands.startJob = async (self, params) => {
      ...
      try {
        await this.uploadAndSetupPlayerJobs(self, job);
        self.logger.info('All files uploaded and set up');
        self.logger.info('Players have begun');
        for(const [playerKey, player] of Object.entries(self.players)) {
    ...
    this.commands.updateRoutine = async (self, params) => {
      if (self.fsm.current === `processingJob`) {
        ...
        for (const [playerKey, player] of Object.entries(self.players)) {
          try {
            ...
            if (jobObject.fsm.current === `complete`) {
              ...
              console.log(self.currentJob.nMetajobsComplete, self.currentJob.nMetajobs);
        ...
        for (const [playerKey, player] of Object.entries(self.metajobCopy)) {
  ...
  async setupConductorArms() {
    ...
    for (const [botKey, bot] of Object.entries(this.app.context.bots.botList)) {
  ...
  async uploadAndSetupPlayerJobs(self, job) {
    self.nJobs = 0;
    ...
    try {
      await new Promise(async (resolve, reject) => {
        ...
        .on(`close`, async () => {
          ...
          await Promise.map(Object.entries(self.metajob), async ([metajobPlayerKey, metajobPlayer]) => {
